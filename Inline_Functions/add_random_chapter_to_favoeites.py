...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    if chapter not in favorites:
        ...
        Users.update_one({"chat_id": Chat_ID}, {"$set": {"favorites": favorites}})

        Send.Callback_Answer(Bot_Variables, "פרק " + str(int_to_gematria(chapter+1)) + " (" + str(chapter+1)+")" + " נוסף למועדפים ✅")
        ...
        Send.Edited_Inline_Keyboard(Bot_Variables, Send_Book.Func(chapter, Bot_Variables),
                             ...
                             Button.Inline(["C", button, "mark_random_as_read"]))
        ...
        Users.update_one({"chat_id": Chat_ID}, {"$set": {"favorites": favorites}})
        Send.Callback_Answer(Bot_Variables, "פרק " + str(int_to_gematria(chapter+1)) + " (" + str(chapter+1)+")" + " הוסר ממועדפים 🚫")
