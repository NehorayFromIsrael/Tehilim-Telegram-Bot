...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    chapters = Names.find_one({"chat_id":chat,"name":men_name})["chapters_stats"]

    if (not chapter in chapters):
        chapters.append(chapter)
        Names.update_one({"chat_id":chat,"name":men_name}, {"$set": {"chapters_stats": chapters}})

    Send.Callback_Alert(Bot_Variables, "פרק " + str(int_to_gematria(chapter+1)) + " (" + str(chapter+1)+")" + " סומן כנקרא ✅")
