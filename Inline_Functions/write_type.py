...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    if write_type == "right to left":
        Users.update_one({"chat_id": Chat_ID}, {"$set": {"write_type": "left to right"}})

    else:
        Users.update_one({"chat_id": Chat_ID}, {"$set": {"write_type": "right to left"}})



    if write_type == "right to left":
        write_type = "➡️ גלילה משמאל לימין️"

    else:
        write_type = "גלילה מימין לשמאל ⬅️"

    Send.Edited_Inline_Keyboard(Bot_Variables, "שינוי הגדרות",
