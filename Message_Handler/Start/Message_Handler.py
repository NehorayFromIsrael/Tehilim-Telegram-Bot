...
import Other_Functions.search_chapter as search_chapter
import Other_Functions.post_name as post_name
import Other_Functions.post_name2 as post_name2
import Other_Functions.delete_name as  delete_name
import Other_Functions.neshama as  neshama
...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    if Virtual_Location == "חפש פרק 🔎" and Message  != "חפש פרק 🔎":
        chapter = Message

        search_chapter.Func(update,context,chapter)


    if Virtual_Location == "post_name" :
        post_name.Func(update,context)

    if Virtual_Location == "post_name2" :
        post_name2.Func(update,context)

    if Virtual_Location == "delete_name" :
        delete_name.Func(update,context)


    if Virtual_Location == "neshama" and Message != "אותיות נשמה 👤" :
        neshama.Func(Bot_Variables)