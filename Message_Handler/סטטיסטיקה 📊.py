...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    users = Users.find({"chat_id": Chat_ID})
    users_len = Users.find({"chat_id": Chat_ID}).count()

    chapters_count = 0
    arrey_chap = []

    for i in range(users_len):
        chapters = users[i]["stats_chapters"]

        for i in range(150):
            try:
             chapters_count = chapters_count + chapters[str(i)]
    ...
    מספר הפרקים שנקראו ע"י כל המשתמשים: {}

    מספר פעמים שסיימו את התהילים: {}
    """.format(chapters_count, t_count)
