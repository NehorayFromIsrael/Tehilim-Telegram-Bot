...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    if is_read_chapters == 0:
        ...
        Send.Inline_Keyboard(Bot_Variables, Send_Book.Func(0,Bot_Variables),
         Button.Inline(["C", "⬅️", rolling_left.Func(chapter,book_chapters,write_type)],["C", favoriets_name.Func(update,context), "Favorite"],["C", "➡️", rolling_right.Func(chapter,book_chapters,write_type)]),
                         Button.Inline(["C", is_read_name.Func(update,context), "is_read"] ) )
    else:
        Send.Inline_Keyboard(Bot_Variables, "כבר התחלת לקרוא כמה פרקים בתהילים מה ברצונך לעשות?" ,
