...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    delete = Names.find_one({"name":Message,"chat_id": Chat_ID})
    if delete == None:
        Send.Message(Bot_Variables, "לא פרסמת שם זה, יש כנראה טעות כתיב")
    else:
        Names.delete_one({"chat_id": Chat_ID,"name":Message})
        Send.Message(Bot_Variables, "☑️ שם נמחק בהצלחה ☑️")
