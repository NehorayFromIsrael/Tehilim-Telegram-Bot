...
import Other_Functions.generate_link as generate_link
import telegram
...
Send.Inline_Keyboard(Bot_Variables, "Text Here" ,
    ...
    Users.update_one({"chat_id": Chat_ID}, {"$set": {"virtual_location" : "post_name2"}})
    seperator = generate_link.Func(Bot_Variables)
    link = "https://t.me/" + telegram.Bot(Bot_Variables[0]).get_me().username + "?start=T" +seperator
    name = {
        "chat_id": Chat_ID,
        "name": Message,
        "for": "",
    ...
    condition = Names.find({"chat_id":Chat_ID,"status": "pending"}).count()>0
    if condition == False:
        Names.insert_one(name)
    else:
        Names.update_one({"chat_id": Chat_ID,"status": "pending"}, {"$set": {"name": Message}})

    Send.Message(Bot_Variables, "כתוב לשם מה תרצה שיקראו על שמו את התהילים, לרפואתו/הצלחתו/עילוי נשמת וכו")
